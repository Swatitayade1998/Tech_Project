...
public class AgentModify extends HttpServlet
{
        ...
        throws ServletException, IOException
    {
        ...
        try
        {
            ...
            pstmt = DBConn.prepareStatement((new StringBuilder("update agents set agentname=?,agentfname=?,agentage=?,agentsex=?,agentqual=?,agentoccupation=?,agentaddress=?,agentsecuritydeposit=?,branchmgrid=? where agentid=")).append(id).toString());
            ...
            rs = DBConn.executeQuery((new StringBuilder("select branchmgrid from branchmgr where branchmgrname='")).append(mbname).append("'").toString());
            if(rs.next())
            {
                mgrid = rs.getInt(1);
